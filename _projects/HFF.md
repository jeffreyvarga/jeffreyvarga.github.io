---
layout: page
title: Ethics in Fire Assessment
description: An analysis and case study of the social and ethical side of post-fire assessment.
img: assets/img/HFF/HFF_Cover.png
importance: 5
redirect: https://arcg.is/1izmne5
tags: ["communication", "ethics"]
related_publications: false
---
View the full Storymap here:

<a href="/workspaces/jeffreyvarga.github.io/assets/pdf/JeffreyVarga_FinalReport.pdf" download>
    Download the report (PDF) 
</a>

<div class="row">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/HFF/HFF_NBR_Main.jpg" title="HFF NBR Main" class="img-fluid rounded z-depth-1" %}
    </div>
</div>
<div class="caption">
    A case study of fire assessment, seen in the StoryMap presentation above.
</div>